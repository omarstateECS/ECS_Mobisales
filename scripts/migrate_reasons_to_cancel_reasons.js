...
async function migrateReasonsToCancelReasons() {
  try {
    console.log('Starting migration from reasons to CancelReasons...\n');
    ...
    reasons.slice(0, 3).forEach(r => {
      console.log(`  - ID: ${r.reasonId}, Description: ${r.description}`);
    });
    console.log('');

    // Insert each reason into CancelReasons table
    let insertedCount = 0;
    let skippedCount = 0;
    ...
    for (const reason of reasons) {
      try {
        ...
        `;
        insertedCount++;
        console.log(`✓ Inserted reason ${reason.reasonId}: ${reason.description}`);
      } catch (error) {
        skippedCount++;
